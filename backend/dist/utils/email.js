...
exports.sendPaymentReceiptEmail = void 0;
...
const escapeHtml = (value) => value
    ...
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#039;");
const sendPaymentReceiptEmail = async (order) => {
    ...
    const itemsHtml = order.items
        .map((item) => `<li>${escapeHtml(item.name)} x${item.quantity} - ${escapeHtml(item.size)} / ${escapeHtml(item.color)}</li>`)
        .join("");
